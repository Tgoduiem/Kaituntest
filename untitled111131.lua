...
-- vd: ["V2 Race"] = true ---sửa--> ["V2 Race"] = "false" -> bú cặc
...
gay = tick()
print("[Nexus / Info] Connecting To Server");

print("[Nexus / Info] Connected To Server"); 

print("[Nexus / Info] Fetching Requirements Data..."); 

print("[Nexus / Info] Saving Data To Workspace"); 

print("[Nexus / Info] Authencating..."); 
wait(.2)
print("[Nexus / Info] Authencation Disabled, Loading Script...");

print("[Nexus / Info] Loaded In "..tostring(tick()-gay).."ms")
if game.Players.LocalPlayer.Name == "Rip_NgaoGaming" then 
  game.Players.LocalPlayer:Kick("You Have Been Permerantly Banned For Reason: Idiot") 
end 
...
script_time = os.time()
if not isfolder(".nexus/BloxFruit/Kaitun/") then
               makefolder(".nexus/BloxFruit/Kaitun/")
end
   ...
   local badtimedicac = os.time()
         if not isfile(".nexus/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name) then
        
         writefile(".nexus/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name, "0")
         esltime = 0 
         end 
   ctent2 = readfile(".nexus/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name)
   function updatetime()
      if not isfile(".nexus/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name) then
         ...
         ctent = readfile(".nexus/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name)
         if tonumber(ctent) or ctent == "0" or ctent == "1" then
            esltime = tonumber(ctent2 or 0) + (os.time()-(badtimedicac or os.time()))
            writefile(".nexus/BloxFruit/Kaitun/eslapedtime."..game.Players.LocalPlayer.Name, tostring(esltime))
         ...
         local AkaliNotif =
         loadstring(game:HttpGet("https://raw.githubusercontent.com/Tgoduiem/Kaituntest/refs/heads/main/Lkaitun.txt"))()
...
local Window = Library:CreateWindow({
    ...
    Title = 'Nexus Community Free Scripts '..os.date("%d".." ".."%B".." ".."%Y"),
...
Info:AddLabel("Nexus Community Kaitun Script") 
 ...
 local decalsyeeted = true
  ...
  --          game.ReplicatedStorage.Effect.Container.Respawn:Destroy()
         ...
         function SaveSettings2()
            local HttpService = game:GetService("HttpService")
            if not isfolder(".nexus/utils/") then
               makefolder(".nexus/utils/")
            end
            writefile(".nexus/utils/" .. bN, HttpService:JSONEncode(bM))
         end
         function ReadSetting2()
            ...
            function()
               ...
               return HttpService:JSONDecode(readfile(".nexus/utils/" .. bN))
            end
            )
            if s then
               return o
    ...
    while (fe and wait()) or task.wait(c().Misc.Webhook["Send While"].Delay) do 
      ...
      if c().Misc.Webhook["Send While"]["Account Status"] then 
        ...
        ditmemememememmeme["embeds"] = {{
          ...
          ["footer"] = {
             ["text"] = "Nexus Cuttay / https://discord.com/invite/kGBwybQw64 / "..disp_time(esltime) or "",
           },
...
--                                           end
                                       ...
                                       until  not dicac.Character:FindFirstChild"Head" or  os.time()-starttime > 30 or not game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible or game.Players.LocalPlayer.PlayerGui.Main.SafeZone.Visibledmlockskill = false
